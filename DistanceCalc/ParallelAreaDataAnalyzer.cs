...
{
    public class ParallelAreaDataAnalyzer : IAreaDataAnalyzer
    {
        private double _distanceRange;
        private int _amountOfThreads;
        private User _user;
        public ParallelAreaDataAnalyzer(double distanceRange, User user,
            int amountOfThreads)
        {
        ...
        {
            /* PLINQ query which calculates distance for each user to the specified new user in parallel,
            ...
            var usersInRange = from user in userList.AsParallel()
                        .WithDegreeOfParallelism(_amountOfThreads)
                        .WithExecutionMode(ParallelExecutionMode.ForceParallelism)
                        where user != null
                        let distance = CalculateDistance(user)
                        where distance <= _distanceRange
                        orderby distance, user.Username
                        select new User
                        {
            ...
            //Filters users once again, this time it checks if email is in valid format. ForAll method
            // executes the query in parallel
            usersInRange.ForAll(u => {
                if(!emailChecker.IsValid(u.Email))
                {
                    usersInRangeList.Remove(u);
                }
            });
